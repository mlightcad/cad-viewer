export default {
  ACAD: {
    hatch: {
      description:
        'Şerit tabanlı bağlamsal iş akışıyla tarama dolgusu oluşturur'
    },
    layer: {
      description: 'Katman özellikleri yöneticisi'
    },
    md: {
      description: 'Eksik yazı tiplerini ve görselleri değiştirir'
    },
    properties: {
      description: 'Varlık özellikleri paleti'
    },
    pttype: {
      description: 'Nokta nesnelerinin görüntü stilini ve boyutunu belirler'
    },
    qselect: {
      description:
        'Tür ve özellik koşullarına göre nesneleri filtreleyerek bir seçim kümesi oluşturur'
    },
    units: {
      description:
        'Doğrusal ve açısal birim biçimlerini, hassasiyeti, açı yönünü ve ekleme ölçek birimlerini ayarlar'
    },
    style: {
      description:
        'Metin ve çok satırlı metin için metin stilleri oluşturur, değiştirir veya belirler'
    }
  },
  USER: {}
}
