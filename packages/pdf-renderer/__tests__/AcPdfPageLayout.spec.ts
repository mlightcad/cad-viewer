...
describe('pdfRebaseOrigin', () => {
  it('skips frames that float32 can still represent', () => {
    expect(pdfRebaseOrigin(box(0, 0, 100, 50))).toBeUndefined()
    expect(
      pdfRebaseOrigin(box(0, 0, PDF_REBASE_THRESHOLD - 1, 10))
    ).toBeUndefined()
  })
